...
def theo_kurve (x,I,R):
    ...
    mu0 = 1.256e-6
    N = 1200
    return (mur * mu0 * N / (2 * np.power((x)**2+ R**2,(2/3))) * R**2 * I)
...
for i in range(len(data)):
    ...
    dataWithErr[i].append(arrToUnumpyf([j*scaler for j in data[i][2]],[np.sqrt((j*0.002*scaler)**2+(j*0.003*scaler)**2)+digitalErr(0.001*scaler) for j in data[i][2]]))

#print(dataWithErr[0])
fig, ax = plt.subplots()
ax.grid()
difference = [[i[1][j]-i[2][j]  for j in range(len(i[1]))] for i in dataWithErr]
x_n , y_n = mirrorDataAroundX(5*scaler,nominal_values(dataWithErr[0][0]),nominal_values(difference[0]))
#print(x_n,y_n)
